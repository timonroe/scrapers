...
const {
  LOGGING_DISPATCH_SCRAPER,
} = process.env;

const NAME = 'The Dispatch';
const SHORT_NAME = 'The Dispatch';
const URL = 'https://thedispatch.com';
const URL_POLITICS = 'https://thedispatch.com/category/politics';

export class DispatchScraper implements NewsScraper {
  ...
  constructor() {
    this.source = NewsScraperSource.DISPATCH;
    this.name = NAME;
    this.shortName = SHORT_NAME;
    this.url = URL;
    this.urlPolitics = URL_POLITICS;
    if (LOGGING_DISPATCH_SCRAPER && LOGGING_DISPATCH_SCRAPER === 'on') {
  ...
  async scrapePolitics(): Promise<NewsScraperResponse> {
    ...
    try {
      ...
      const headlineElements = $('h2.text-brand-natural-black');
      ...
      this.logger.error('DispatchScraper.scrape error: %s', error.message);
      throw error;
    }
    const response = {
    ...
    this.logger.verbose('DispatchScraper.scrape: %s', JSON.stringify(response, null, 2));
    return response;
  }
